...
parser.add_argument('--drop_rate', type=float, nargs=2,
	help='Drop rate of each Dropout Layer')
...
parser.add_argument('--conv_ch', type=int, nargs=2, required=True,
	help="Conv channel size.")
parser.add_argument('--conv_ker', type=int, nargs=2, required=True,
	help="Conv ker size.")
parser.add_argument('--pool_ker', type=int, nargs=2, required=True,
	help="MaxPooling kernel size.")
parser.add_argument('--pool_stride', type=int, nargs=2, required=True,
	help="MaxPooling stride.")
parser.add_argument('--weight_decay', type=float, default=0,
	help='Weight decay')
parser.add_argument('--name', required=True, type=str,
	help="Name of the experiment for wandb")
parser.add_argument('--wb_project_prefix', default='ann_2', type=str,
...
if __name__ == '__main__':
	...
	if args.is_train:
		...
		cnn_model.to(device)
		print(cnn_model)
		print("Parameter Size", sum(map(torch.numel, cnn_model.parameters())) / 1000 / 1000, "M")
		optimizer = optim.AdamW(cnn_model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
			...
			if val_acc >= best_val_acc:
				...
				if not args.dr:
					wandb.log({
						"test/loss": test_loss,
						"test/acc": test_acc,
						"test/best_val_acc": best_val_acc,
					}, step=epoch)
