...
class TfmrLMHeadModel(nn.Module):
    ...
    def batch_loss(self, logits, labels, loss_func, PAD_ID, eps=1e-10):
        assert logits.shape[:2] == labels.shape, f"{logits.shape} , {labels.shape}"

        # Filter input
        shift_logits = logits[..., :-1, :].contiguous()
        shift_labels = labels[..., 1:].contiguous()

        # Define Mask: 1 means selected as a loss term
        position_mask = torch.ones_like(labels, dtype=torch.float)
        position_mask[:, 1:] = (shift_labels != PAD_ID).float()
        position_mask = position_mask[:, :-1]

        # Calculate Loss
        element_loss = loss_func(shift_logits.transpose(1, 2), shift_labels)
        batch_loss = (element_loss * position_mask).sum(dim=1) / (position_mask.sum(dim=1) + eps)
        return batch_loss
    ...
    def inference(self, device, PAD_ID, batch_size, maxlen, decode_strategy, temperature, top_p=1.0, top_k=50267):
        ...
        allgen = []
        with torch.no_grad():
            for i in trange(0, int(5000/batch_size)+1, desc="Inference"):
